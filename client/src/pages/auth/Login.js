...
useEffect(() => {
    ...
    const rolebasedredirect = ((result) => {
        if(result.data.finduser.role === "admin"){
        ...
        <div className="container p-5">
            ...
            <div className="row">
                <div className="col-md-6 offset-md-3">
                    {loading?<h4 className=" $danger" style={{color:"red"}}>Loading...</h4>:<h4 className=" $danger" >Login</h4>}
                    {LoginForm()}
                    <Button
                        onClick={Logingoogle}
                        type="danger submit"
                    ...
                    <Link to="/forgot/password" className="text-danger p-3 float-end" >Forgot Password ?</Link>
                </div>
            </div>
        </div>
    )
